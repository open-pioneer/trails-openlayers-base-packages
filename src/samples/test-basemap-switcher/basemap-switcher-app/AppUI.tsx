...
import { ImageBasemapSwitcher } from "@open-pioneer/image-basemap-switcher";
import {
    Box,
    Button,
    Flex,
...
import { BaselayerImageBasemapSwitcherProps } from "@open-pioneer/image-basemap-switcher/ImageBasemapSwitcher";

export function AppUI() {
    const intl = useIntl();

    const myMapping: Map<string, BaselayerImageBasemapSwitcherProps> = new Map([
        ["myLayerId", { image: "myImage", label: "myLabel" }]
    ]);

    return (
        <Flex height="100%" direction="column" overflow="visible">
            <TitledSection
                title={
                ...
                <Flex flex="1" direction="column" position="relative">
                    <MapContainer mapId={MAP_ID}>
                        <MapAnchor
                            ...
                            horizontalGap={10}
                            verticalGap={10}
                            className={"map-anchor-visibility"}
                        >
                            <Box
                            ...
                            >
                                <FormControl>
                                    ...
                                    <Flex m={2} direction={"column"} gap={2}>
                                        <Text as="b">
                                            {intl.formatMessage({ id: "alternativeBasemapLabel" })}
                                        </Text>
