...
it("returns the same function across re-renders", () => {
    const { result, rerender } = renderHook(() => useSelectionSourceId());
...
function renderIdHook() {
    return renderHook(() => useSelectionSourceId()).result.current;
}

function createSource(label: string, id?: string): SelectionSource {
    return { id, label, select: async () => [] };
}
